...
class ProjectsController extends Controller{
    ...
    public function index(Request $request){
      //gak ada keyword dan filter defaultnya gini
       if (empty($request->keywords) && empty($request->filter) ){
        $jobs = Job::orderBy('job_id','DESC')->where('active',1)->with(['harusbisaskill','harusbisaskill.skills'])->paginate(5);
        if(!empty($request->min_price)){
          $jobs = Job::orderBy('job_id','DESC')->where('price_min','>=',$request->min_price)->where('price_max','<=',$request->max_price)->where('active',1)->with(['harusbisaskill','harusbisaskill.skills'])->paginate(5);
        }
      }
      //gak ada keyword tapi ada filter//
        ...
        if ($request->filter === "newest-first")
          ...
          $jobs = Job::orderBy('price_max','ASC')->where('active',1)->with(['harusbisaskill','harusbisaskill.skills'])->paginate(5);
        else if($request->filter === "highest-budget-first")
          $jobs = Job::orderBy('price_max','DESC')->where('active',1)->with(['harusbisaskill','harusbisaskill.skills'])->paginate(5);
        else if($request->filter === "lowest-bid")
          $jobs = Job::orderBy('no_of_bids','ASC')->where('active',1)->with(['harusbisaskill','harusbisaskill.skills'])->paginate(5);
        else if($request->filter === "highest-bid")
          $jobs = Job::orderBy('no_of_bids','DESC')->where('active',1)->with(['harusbisaskill','harusbisaskill.skills'])->paginate(5);
      }
      //ada keyword dan filter
        ...
        if ($request->filter === "newest-first")
          ...
          ->where('active',1)->with(['harusbisaskill','harusbisaskill.skills'])->paginate(5);
        else if ($request->filter === "lowest-budget-first")
      ...
      //ada keyword gak ada filter
        ...
        ->where('active', 1)->where('price_max','<=',$request->max_price)->with(['harusbisaskill','harusbisaskill.skills'])->paginate(5);
      }
    ...
    public function viewProject($slug){
      ...
      $skills = DB::table('skills')
                ...
                ->select('harus_bisa_skill.skills_id', 'skills.name', 'harus_bisa_skill.job_id')
                ->get();
